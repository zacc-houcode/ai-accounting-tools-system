...
| 工具 | 做什么 |
|---|---|
| 🛠️ 工作流程建造器 | 画流程：模块、步骤、或者（Or）、循环（Loop），标 Audit 点，写清楚谁负责、要什么工具和资料。可以放大看、导出 PNG 或 JSON |
| 🤖 AI Agent 团队建造器 | 部门长期用的那一支团队：5 个现成 Agent（指挥、连接、流程、技能、训练），再加上你自己的岗位。导出 `AI-TEAM-SYSTEM.md` |
